...
describe('favorite blog', () => {
  ...
  test('of one blog to equal that one', () => {
    expect(listHelper.favoriteBlog(oneBlog)).toEqual(oneBlog[0])
  })
  test('of many blogs is the right one', () => {
    expect(listHelper.favoriteBlog(blogs)).toEqual(blogs[2])
  })
})

describe('most blogs', () => {
  test('of emmpty list is null', () => {
    expect(listHelper.mostBlogs([])).toBe(null)
  })
  test('of one blog to equal that ones author', () => {
    const result = listHelper.mostBlogs(oneBlog)
    expect(result.author).toBe('Michael Chan')
    expect(result.blogs).toBe(1)
  })
  test('of many blogs is the right author', () => {
